...
import { Orientation, Tetracoordinate, CartesianCoordinate } from "tetracoord/src/tetracoord/vector"
import { TetracoordSpace, TetracoordCell } from "tetracoord/src/tetracoord/tetracoord_space"
...
function ExplorerCanvas(props: ExplorerCanvasProps) {
  ...
  React.useEffect(() => {
    ...
    const tspace = new TetracoordSpace(
      Orientation.UP,
      UNIT_PX,
      [paper.view.bounds.center.x, paper.view.bounds.center.y]
    )
    ...
    paper.view.onMouseUp = function (mouse: paper.MouseEvent) {
      ...
      let tcell = tspace.ccoord_to_cell(new CartesianCoordinate(m.x, m.y))
